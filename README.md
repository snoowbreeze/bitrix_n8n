...
- **Контактные поля** (Телефон, E-mail, Сайт, Мессенджер) доступны как отдельные
  удобные поля. Достаточно ввести значение, например `+77771234567`. Можно задать
  тип значения префиксом (`MOBILE:+7777...`, `HOME:mail@x.ru`) и указать несколько
  значений — каждое с новой строки. При отправке они автоматически собираются в
  множественное поле `fm` в формате, который ожидает Битрикс24.
