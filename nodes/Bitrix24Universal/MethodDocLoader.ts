...
import apidocsMethodUrls from './apidocs-method-urls.json';

const MCP_URL = 'https://mcp-dev.bitrix24.com/mcp';
const APIDOCS_BASE = 'https://apidocs.bitrix24.ru';
const methodUrlIndex = apidocsMethodUrls as Record<string, string>;

const paramsCache = new Map<string, ResourceMapperField[]>();
...
function shortDescription(text: string, maxLen = 110): string {
...
function buildDisplayName(id: string, typeStr: string, description: string, required: boolean): string {
	const desc = shortDescription(description);
	const typePart = typeLabel(typeStr);
	const requiredMark = required ? ' *' : '';
	if (desc) return `${id}${requiredMark} (${typePart}) — ${desc}`;
	return `${id}${requiredMark} (${typePart})`;
}

function methodToSlug(method: string): string {
	return method.trim().toLowerCase().replace(/\./g, '-');
}

function buildApidocsCandidates(method: string): string[] {
...
function findParamsTableStart(html: string): number {
	...
	while (cursor < searchArea.length) {
		...
		if (tableHeader === -1) return -1;

		const hasTypeHeader =
			searchArea.includes('>тип</code>', tableHeader) ||
...
function extractParamsSection(html: string): string {
	const startMarkers = [
...
export function parseParamsFromApidocsHtml(html: string): ResourceMapperField[] {
	const fields: ResourceMapperField[] = [];
	const section = extractParamsSection(html);
	const paramPattern =
		/<strong>([A-Za-z_][A-Za-z0-9_]*)<\/strong>(\*?)<br\s*\/?>\s*(?:<a[^>]*>)?<code[^>]*>([^<]+)<\/code>/g;
	...
	for (const match of section.matchAll(paramPattern)) {
		...
		fields.push(
			mapperField(id, buildDisplayName(id, typeStr, description, required), required, type, {
				...(options ? { options } : {}),
...
async function fetchParamsFromApidocs(
		...
		try {
			...
			const body =
				typeof response === 'string'
					? response
					: typeof (response as IDataObject).body === 'string'
						? ((response as IDataObject).body as string)
						: String((response as IDataObject).data ?? '');
 ...
 */
export function parseMethodParamsText(text: string): ResourceMapperField[] {
	const fields: ResourceMapperField[] = [];
	...
	for (const rawLine of text.split('\n')) {
		...
		const match = line.match(
			/^([A-Za-z_][A-Za-z0-9_]*):\s*\(([^)]+)\)\s*(.*?)\s*\[required:\s*(yes|no|да|нет)\]\s*$/i,
		);
		if (!match) continue;
...
function extractTextFromMcpSse(body: string): string {
	for (const line of body.split('\n')) {
		if (!line.startsWith('data: ')) continue;

		try {
			const payload = JSON.parse(line.slice(6)) as IDataObject;

			const structured = (payload.result as IDataObject)?.structuredContent as IDataObject;
			if (typeof structured?.result === 'string') {
				return structured.result;
			}

			const content = (payload.result as IDataObject)?.content as IDataObject[];
...
async function fetchParamsFromMcp(
	context: ILoadOptionsFunctions | IExecuteFunctions,
	method: string,
): Promise<ResourceMapperField[]> {
	...
	const body =
		typeof response === 'string'
			...
			: typeof (response as IDataObject).body === 'string'
				? ((response as IDataObject).body as string)
				: String((response as IDataObject).data ?? response);

	const paramsText = extractTextFromMcpSse(body);
	if (!paramsText || paramsText.toLowerCase().includes('not found')) return [];

	return parseMethodParamsText(paramsText);
}

export async function fetchMethodParamsFromDocs(
	context: ILoadOptionsFunctions | IExecuteFunctions,
	method: string,
): Promise<ResourceMapperField[]> {
	const cacheKey = method.trim().toLowerCase();
